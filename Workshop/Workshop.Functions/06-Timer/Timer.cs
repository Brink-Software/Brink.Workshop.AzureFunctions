using System;
using System.IO;
using System.Threading.Tasks;
using Azure.Storage.Blobs;
using Azure.Storage.Blobs.Specialized;
using Microsoft.Azure.WebJobs;
using Microsoft.Extensions.Logging;

namespace Workshop.Functions
{
    //  https://arminreiter.com/2017/02/azure-functions-time-trigger-cron-cheat-sheet/
    public class Timer
    {
        [FunctionName("Timer")]
        public async Task Run([TimerTrigger("* * * * * 0")]TimerInfo myTimer, ILogger log)
        {
            var blobContainerClient = new BlobContainerClient("UseDevelopmentStorage=true", "sample-container");
            try
            {
                await blobContainerClient.CreateIfNotExistsAsync();
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
                throw;
            }
           
            var blockBlobClient = blobContainerClient.GetBlockBlobClient("DemoData");
            using (var ms = new MemoryStream())
            {
                var writer = new StreamWriter(ms);
                writer.Write(Guid.NewGuid());
                await writer.FlushAsync();
                ms.Position = 0;
                await blockBlobClient.UploadAsync(ms);
            }

            log.LogInformation($"C# Timer trigger function executed at: {DateTime.Now}");
        }
    }
}
